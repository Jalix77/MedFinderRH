...
test.describe('Erreurs, etats vides et coherence UI/backend', () => {
  ...
  test('le montant accorde affiche a l\'ecran correspond exactement a la valeur en base (aucune valeur fictive)', async ({ page }) => {
    const { grantId, grantName } = await createGrant(`consist${Date.now()}`)
  ...
  test('budget fraichement cree : etat vide honnete (aucune ligne), pas un tableau vide sans explication', async ({ page }) => {
    ...
    const { data: fy } = await admin
      .from('fiscal_years')
      .insert({ organization_id: orgResp.data!.id, label: `E2E-empty-${Date.now()}`, start_date: '2033-01-01', end_date: '2033-12-31' })
      .select('id')
      .single()
    const { data: budget } = await admin
      .from('budgets')
      .insert({ organization_id: orgResp.data!.id, fiscal_year_id: fy!.id, name: `Budget E2E vide ${Date.now()}` })
      .select('id')
      .single()
